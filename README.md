...
> **Winner of the #1 Overall Prize for Best Use of the Confluent Platform** at the [Confluent AI Day NYC Hackathon](https://events.confluent.io/confluentaiday2026nyc) (September 2026).
>
> Built with **Confluent Cloud**, **Apache Flink SQL**, **Confluent Schema Registry**, **Managed HTTP Source Connectors**, and **In-Stream & Interactive AI**. The in-Flink dispatcher agent runs on AWS Bedrock or Google Gemini; the three interactive copilots use Google Gemini when a Google API key is set and fall back to Anthropic Claude otherwise. An optional, best-effort Real-Time Context Engine (RTCE / MCP) integration is available separately via `scripts/setup_rtce.py`.
...
The dashboard is a high-performance live map: real subway route lines (from static GTFS geometry) with live subway trains and **~2,700 city buses** — buses carry real GPS lat/lon + heading — moving on top, plus mode/route filters and on-map alert markers. Alongside the map, three **interactive AI copilots** (Rider Advisor, Fleet Risk Predictor, and Bus Route Designer) answer questions grounded in the live system state — they use Google Gemini when a Google API key is set and fall back to Anthropic Claude otherwise.
...
| `mta_bus_positions` | Python Producer | Live bus GPS (real lat/lon + heading), ~2,700 buses |
...
Released under the [MIT License](LICENSE). Copyright (c) 2026 Josue Villanueva.
